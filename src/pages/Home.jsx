...
const Home = () => {
  ...
  if (loading) return (
    <div className="h-screen w-full bg-[#3E2723] flex flex-col items-center justify-center">
      <motion.h1 animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 2, repeat: Infinity }} className="text-[#D4AF37] font-serif italic text-6xl mb-4 text-center px-6">Aarahi</motion.h1>
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF5] text-[#3E2723] font-serif overflow-x-hidden pb-32">
      ...
      <section className="relative h-screen flex items-center justify-center text-center px-6 bg-[#F5F5DC] overflow-hidden">
        <div className="absolute inset-0 opacity-10"><img src="/Aarahi Image 3.jpg" className="w-full h-full object-cover scale-105" alt="Pure Flour" /></div>
        <div className="max-w-6xl relative z-10">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-3 bg-white/90 px-6 py-2 rounded-full border border-[#D4AF37]/20 shadow-xl mb-10">
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-7xl md:text-[9rem] font-bold mb-8 leading-[0.85] tracking-tighter uppercase">
            Stop Eating <br /><span className="italic text-[#D4AF37] font-light normal-case">"Dead"</span> Flour.
          </motion.h1>

          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-xl md:text-2xl mb-12 font-sans text-gray-700 max-w-2xl mx-auto leading-relaxed font-light">
          ...
          <Link to="/shop">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="bg-[#3E2723] text-white px-16 py-6 rounded-full text-lg font-black shadow-2xl uppercase tracking-widest transition-all">Claim Freshness</motion.button>
          </Link>
        </div>
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto grid md:grid-cols-3 gap-8">
        {[
          { icon: <Wind size={20}/>, t: "Cold-Milled", d: "Traditional stones maintain a cool 28°C process." },
          { icon: <Star size={20}/>, t: "Sharbati Gold", d: "Sourced directly from MP's finest wheat belts." },
          { icon: <Zap size={20}/>, t: "24Hr Cycle", d: "Milled on demand. Arrives while the life is peaking." }
        ].map((item, i) => (
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} key={i} className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all text-left">
      ...
      <section className="py-24 bg-[#FCF9F2] px-6 text-left">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            ...
            <div className="p-8 bg-white rounded-[2.5rem] shadow-xl border-l-[10px] border-[#D4AF37]">
              ...
              <p className="mt-4 text-gray-500 font-sans text-sm italic">Unlike industrial rollers that hit 90°C, our stones keep the heat low and the oils alive.</p>
            </div>
          </motion.div>
      ...
      <section className="py-32 px-6">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-5xl mx-auto text-center">
          ...
          <div className="flex flex-col items-center">
            <div className="w-20 h-20 rounded-full bg-gray-100 mb-4 overflow-hidden border-2 border-[#D4AF37]/20 flex items-center justify-center">
               <span className="text-[10px] text-gray-400 font-sans uppercase">Portrait</span>
            </div>
      ...
      <section className="py-24 bg-[#FCF9F2] px-6">
        ...
        <div className="max-w-4xl mx-auto space-y-10 text-left">
          {[
            { t: "05 AM", h: "Selection", d: "Hand-picking the finest Sharbati grains in Bengaluru." },
            { t: "09 AM", h: "Milling", d: "Cold-milling begins. Heat is monitored every 15 minutes." },
            { t: "04 PM", h: "Delivery", d: "Dispatched. Reaches your kitchen while the flour is still warm." }
          ].map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="flex items-center gap-10 bg-white p-10 rounded-[2.5rem] shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[92%] z-[9999] md:hidden">
        <div className="bg-white/80 backdrop-blur-2xl border border-white/40 rounded-[2.5rem] px-8 py-4 flex justify-between items-center shadow-[0_25px_50px_rgba(0,0,0,0.15)]">
